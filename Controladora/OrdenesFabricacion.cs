...
namespace Controladora
{
    public class OrdenesFabricacion
    {
        ...
        {
            var ordenes = from orden in oFabrica.ORDEN_FABRICACION.Include(a => a.PEDIDO.CLIENTE.LOCALIDAD).Include(b => b.USUARIO).Include(c => c.LINEA_OF).Include(d => d.PEDIDO.LINEA_PEDIDO.Select(e => e.PRODUCTO.LINEA_PRODUCTO.Select(f => f.INSUMO))).Include(g => g.ESTADO).Include(h => h.PEDIDO.TAREAS).ToList()
                          select new
                          {
                              ...
                              PEDIDO = orden.PEDIDO,
                              ESTADO = orden.ESTADO,
                              FECHA_CONFECCION = orden.FECHA_CONFECCION,
                              FECHA_ENTREGA = orden.FECHA_ENTREGA                          
                          };
            return ordenes.ToList();
        }
        public System.Collections.IEnumerable LISTAR_OF_FABRICA()
        {
            var ordenes = from orden in oFabrica.ORDEN_FABRICACION.Include(a => a.PEDIDO.CLIENTE.LOCALIDAD).Include(b => b.USUARIO).Include(c => c.LINEA_OF).Include(d => d.PEDIDO.LINEA_PEDIDO.Select(e => e.PRODUCTO.LINEA_PRODUCTO.Select(f => f.INSUMO))).Include(g => g.ESTADO).Include(h => h.PEDIDO.TAREAS).ToList()
                          where (orden.ESTADO.NOMBRE != "CONFECCIONADA")
                          select new
                          {
                              CODIGO = orden.CODIGO_OF,
                              ENCARGADO = orden.USUARIO,
        ...
        public System.Collections.IEnumerable ARMAR_INSUMOS(List<Modelo.Insumo> Insumos) //ordeno y agrupo los insumos a mostrar
        {
            var productos = from linea in Insumos.ToList()
                            group linea by linea.CODIGO into oInsumo
                            select new
                            {
                                oInsumo.FirstOrDefault().CODIGO,
                                oInsumo.FirstOrDefault().CODIGO_INSUMO,
                                oInsumo.FirstOrDefault().NOMBRE_INSUMO,
                                oInsumo.FirstOrDefault().DETALLE,
                                CANTIDAD = oInsumo.Sum(x => Convert.ToInt32(x.CANTIDAD))
                            };
